// Legacy:
//   Route: /sales/invoice-view  → <SalesInvoice />  (allRoutes.js:413)
//   SalesInvoice = withModule(Invoice)              (WithModuleRoutes.js:409)
//   Invoice from craft-frontend/src/Components/PayableReceivableManagement/InVoice/Invoice.js
//
// Behaviour at /sales/invoice-view (per Invoice.js useEffect on location.pathname):
//   pathname includes neither "payable" / "receivable" / "incentive"
//   → fallback branch `setEstimateMode("PAYABLE")`.
//   So the Sales Invoice View screen runs the PAYABLE-mode Partner column set.
//
...
export interface InvoiceRow {
  ...
  id?: string | number;
  invoiceNo?: string;
  // Who (Partner branch — coreEmployeeList only used for EMPLOYEE_INCENTIVE).
...
// Lookups for the filter bar (Territory Type / Territory / Channel / Month).
