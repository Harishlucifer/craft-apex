...
export default function PayoutPlanListPage() {
  ...
  const isIncentive = userType === "EMPLOYEE";
  const createPath = isIncentive
    ? "/finance/add-incentive-plan"
    : "/finance/add-payout-plan";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button asChild>
          <Link to={createPath}>
            <Plus className="h-4 w-4" /> Add {isIncentive ? "Incentive" : "Payout"} Plan
          </Link>
        </Button>
      </div>
