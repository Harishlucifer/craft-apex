...
import {
  useEmployeeList,
  useSalesIncentiveEarnings,
  useTerritoryMaster,
  useTerritoryUser,
} from "./sales-incentive-earnings.api";
import type {
  EstimateFilters,
  EstimateRow,
  SelectOption,
...
// Legacy: craft-frontend/src/pages/IncentiveModule/Earnings.js
...
// Filters from EstimateForm INCENTIVE branch: Territory Type *, Territory *, Employee *, Month.
// Deferred items mirror payable-estimate.page.tsx:
//   - "Calculate Estimate" / "Calculate Earnings" / "Generate Invoice" buttons
//     (legacy posts to GET_ESTIMATE / GET_PAYABLE with side-effect Swal modals)
//   - Single-option auto-select (left manual; legacy did via EstimateForm useEffect)
...
export default function SalesIncentiveEarningsPage() {
  ...
  const { data: payload, isFetching } = useSalesIncentiveEarnings(
    {
      category,
    ...
    <div className="mx-auto max-w-7xl space-y-5">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">
          Sales Incentive Earnings
        </h1>
        <p className="text-sm text-slate-500">
          Earnings estimate for an employee by territory and month.
        </p>
      </div>
        ...
        <CardContent>
          ...
          >
            ...
            <div className="md:col-span-4 flex items-center justify-end gap-2">
              {/* TODO: legacy "Calculate Earnings" / "Generate Invoice" buttons
                  (POST GET_PAYABLE then GET_ESTIMATE with Swal feedback). */}
              <button
                type="submit"
      ...
      {/* Summary cards — legacy non-"Earnings" variant (moduleName="EMPLOYEE" !==
