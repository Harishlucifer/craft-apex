...
// Legacy fetchEstimateList (mode hardcoded to "PAYABLE" in Estimate.js line ~125,
// moduleName === "EMPLOYEE" branch — uses user_type=EMPLOYEE & associate_id=employee_id):
...
export function useIncentiveEstimate(
...
export function useTerritoryMaster() {
  return useQuery({
    queryKey: ["incentive-estimate-list.territory-master"],
...
export function useTerritoryUser() {
  return useQuery({
    queryKey: ["incentive-estimate-list.territory-user"],
...
export function useEmployeeList(territoryId: string) {
  return useQuery({
    queryKey: ["incentive-estimate-list.employee-list", territoryId],
