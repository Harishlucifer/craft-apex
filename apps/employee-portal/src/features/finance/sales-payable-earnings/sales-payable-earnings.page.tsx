...
import {
  useChannelList,
  useSalesPayableEarnings,
  useTerritoryMaster,
  useTerritoryUser,
} from "./sales-payable-earnings.api";
import type {
  EstimateFilters,
  EstimateRow,
  SelectOption,
...
// Legacy: craft-frontend/src/pages/IncentiveModule/ParnterEarnings.js
//   <EstimateComponent module={module} moduleName="CHANNEL_ESTIMATE"/>
// Estimate.js renders the non-"Earnings" branch (moduleName !== "Earnings"),
// so we mirror the 3-card summary variant: Earnings To Date / Lead Count / Payable Count.
// Filters from EstimateForm PAYABLE branch: Territory Type *, Territory *, Channel *, Month.
// Deferred items mirror payable-estimate.page.tsx:
//   - "Calculate Estimate" / "Generate Invoice" buttons (POST GET_PAYABLE / GET_ESTIMATE,
//     Swal feedback)
//   - Single-option auto-select handled below (matches EstimateForm useEffect)
...
const labelFromRow = (r: EstimateRow): string =>
  r.partner_name ?? r.employee_name ?? "-";

export default function SalesPayableEarningsPage() {
  ...
  const { data: payload, isFetching } = useSalesPayableEarnings(
    {
      category,
    ...
    <div className="mx-auto max-w-7xl space-y-5">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">
          Partner Receivable Earnings
        </h1>
        <p className="text-sm text-slate-500">
          Earnings estimate for an approved partner by territory and month.
        </p>
      </div>

      {/* Filter bar — verbatim labels from EstimateForm.js (PAYABLE branch). */}
      <Card>
        <CardHeader>
        ...
        <CardContent>
          ...
          >
            ...
            <div className="md:col-span-4 flex items-center justify-end gap-2">
              {/* TODO: legacy "Calculate Estimate" / "Generate Invoice" buttons. */}
              <button
                type="submit"
      ...
      {/* Summary cards — legacy non-"Earnings" variant (moduleName="CHANNEL_ESTIMATE"
          !== "Earnings"): Earnings To Date, Lead Count, Payable Count.  */}
      {submitted && (
      ...
      <DataTableShell
        ...
        emptyTitle={
          submitted
            ? "No earnings records"
            : "Apply filters to load earnings"
        }
        emptyDescription={
          submitted
            ? "Try a different territory, channel, or month."
            : "Select Territory Type, Territory, and Channel, then click Filter."
        }
        pagination={{
          page,
          totalPages,
