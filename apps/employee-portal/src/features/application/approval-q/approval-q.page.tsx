...
import { useApprovalQ } from "./approval-q.api";
import type { TrackingParticipant } from "../tracking-q/tracking-q.types";
...
const participant = (p?: TrackingParticipant) =>
  !p
    ? "-"
...
/**
 * Lead Approval Queue. Legacy `/pages/Application/ApprovalQ.js` shares the
 * `/alpha/v1/application/tracking` endpoint with TrackingQ but pins
 * `status=-2`. The legacy table also has filter modal + export + workflow
 * summary drawer + notes sidebar — those are deferred until they are needed.
 */
export default function ApprovalQPage() {
  const [page, setPage] = useState(1);
  const { data, isFetching } = useApprovalQ(page);
  ...
  return (
    <DataTableShell
      ...
      emptyTitle={t("approvalQ.emptyTitle")}
      pagination={{
        page,
        totalPages,
        total,
