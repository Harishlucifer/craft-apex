...
import { useDisbursedQ } from "./disbursed-q.api";
import type { DisbursedParticipant } from "./disbursed-q.types";
...
const participant = (p?: DisbursedParticipant) =>
  !p
    ? "-"
    ...
    >
      {rows.map((r, i) => (
        ...
        >
          ...
          <TableCell>
            {r.disbursed_details?.offer_amount
              ? `${r.disbursed_details.offer_type ?? ""} ${fmtAmount(
                  r.disbursed_details.offer_amount
                )}`
              : "-"}
          </TableCell>
