...
import { useEmployerUploadList } from "./employer-upload-list.api";
import type { EmployerUploadRow } from "./employer-upload-list.types";

export default function EmployerUploadListPage() {
  const { data = [], isFetching } = useEmployerUploadList();
  const list = useClientList<EmployerUploadRow>(data, (r, q) =>
    [r.lender_name, r.loan_type, r.file_name].some((v) =>
      ...
      <DataTableShell
        ...
        header={
          <TableRow className={TABLE_HEADER_ROW_CLASS}>
            ...
            <TableHead className={TABLE_HEAD_CLASS}>Process Status</TableHead>
          </TableRow>
        }
      >
        {list.paged.map((r, i) => (
          <TableRow
            key={`${String(r.upload_id ?? r.file_name ?? "")}-${i}`}
          ...
          >
            ...
            <TableCell>
              <Badge variant="secondary">{r.process_status ?? "—"}</Badge>
            </TableCell>
          </TableRow>
        ))}
      </DataTableShell>
