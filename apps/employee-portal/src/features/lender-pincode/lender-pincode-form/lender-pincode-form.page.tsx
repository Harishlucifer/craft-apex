...
export default function LenderPincodeFormPage() {
  ...
  const save = useSavePincodeUpload();
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await save.mutateAsync({
        lender_id: lenderId,
        lender_template_type: configMethod,
      ...
      >
          ...
          {templateLink?.link ? (
            <>
              <p className="mb-3 text-sm font-medium text-slate-700">
                Please download and follow the file structure of upload
                pincodes (.csv / .xlsx)
              </p>
              <a
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-4">
          <Button asChild type="button" variant="outline">
            <Link to="/settings/lender/pin-code/list">Back</Link>
          </Button>
