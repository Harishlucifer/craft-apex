...
import { useMarketingMedia } from "./media-list.api";
import type { MediaRow } from "./media-list.types";

export default function MarketingMediaListPage() {
  const { data = [], isFetching } = useMarketingMedia();
  const list = useClientList<MediaRow>(data, (r, q) =>
    [r.title, r.media_type, r.media_tags].some((v) =>
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {list.paged.map((r, i) => (
          <TableRow
            key={`${String(r.media_id ?? r.title ?? "")}-${i}`}
          ...
          >
            ...
            <TableCell>{r.media_type ?? "—"}</TableCell>
            <TableCell className="text-xs text-slate-500">
              {Array.isArray(r.media_tags)
                ? (r.media_tags as unknown as string[]).join(", ")
                : r.media_tags ?? "—"}
            </TableCell>
